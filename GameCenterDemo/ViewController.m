...
- (IBAction)LoginBTNTouched:(id)sender {
    ...
    [[GameCenterTool defaultTool] getGameCenterAccountWithBlock:^(bool success, UIViewController *viewcontroller) {
         ...
         if(success) {//GC已登陆成功
             NSLog(@"GC已登陆成功");
             NSLog(@"GC 的 ID 是:%@",[GameCenterTool defaultTool].Player.playerID);
...
- (IBAction)GetAllLeaderBoardsBTNTouched:(id)sender {
    //得到所有的排行榜
    [[GameCenterTool defaultTool]GetAllLeaderBoardWithBlock:^(bool success, NSArray *array) {
         if(success){
...
- (IBAction)ShowLeaderBoardBTNTouched:(id)sender {
    //得到排行榜VC
    //type: today,week,all
    UIViewController *vc = [[GameCenterTool defaultTool]GetLeaderBoardVCWithLeaderboardID:kLeaderboardID  andType:@"all"];
    
    //弹出VC
    [self presentViewController:vc animated:YES completion:nil];
    
    //检测排行榜VC关闭了,会在排行榜关闭时触发
    [GameCenterTool defaultTool].LeaderBoardClosedBlock = ^(BOOL closed){
...
- (IBAction)sendSocreBTNTouched:(id)sender {
    ...
    if(socre != 0){
        ...
        //score:分数
        //leaderboardID:排行榜ID
        [[GameCenterTool defaultTool] setRaitScoreWithSocre:[NSNumber numberWithInt:socre]
...
- (IBAction)downloadScoreBTNTouched:(id)sender {
    ...
    //begin: 从第几名开始1~100
    //number:获取多少名用户1~100
    [[GameCenterTool defaultTool]downloadLeaderBoardScoreWithLeaderBoardID:kLeaderboardID andType:@"all"
                                                                andBeginRank:@1
                                                                   andNumber:@10
...
- (IBAction)GetAchievmentBTNTouched:(id)sender {
    ...
    [[GameCenterTool defaultTool]reportAchievment:kAchievementID
...
- (IBAction)getOnlineFriendesBTNTouched:(id)sender {
    [[GameCenterTool defaultTool] getOnlineFriendsWithArrBlock:^(bool success, NSArray *array) {
        if(success){
