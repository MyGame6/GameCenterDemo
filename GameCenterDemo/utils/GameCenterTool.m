...
@interface GameCenterTool ()<GKGameCenterControllerDelegate>

@end

@implementation GameCenterTool

//单例
static GameCenterTool *staticTool;
/**
 *  单例
 *
 *  @return 单例
 */
+(GameCenterTool *)defaultTool{
    if(!staticTool){
        staticTool = [GameCenterTool new];
        [staticTool setup];
    }
    return staticTool;
}
...
- (void)getOnlineFriendsWithArrBlock:(ArrBlock)ArrBlock {
    ...
    [self.Player loadRecentPlayersWithCompletionHandler:^(NSArray<GKPlayer *> * _Nullable recentPlayers, NSError * _Nullable error) {
        
        if (error == nil) {
            ArrBlock(YES, recentPlayers);
        }else {
            NSLog(@"获取已登录的好友列表失败：%@", error);
            ArrBlock(NO, nil);
        }
